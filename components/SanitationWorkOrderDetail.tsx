...
import {
  ...
  ShieldCheck, UserCheck, Lock, Unlock, ChevronRight,
  Activity, Zap, Image as ImageIcon,
...
const STATUS_COLOR: Record<string, string> = {
  pending: HUD.textDim,
  in_progress: HUD.cyan,
  awaiting_qa: HUD.amber,
  completed: HUD.green,
  cancelled: HUD.red,
};
  ...
  return (
    <View style={inp.wrap}>
      <Text style={inp.label}>
        {label}
        {required && <Text style={{ color: HUD.red }}> *</Text>}
      </Text>
      <TextInput
        style={[inp.input, {
          borderColor,
          color: editable ? HUD.text : HUD.textSec,
      ...
      />
      {isEmpty && editable && (
        <Text style={inp.err}>This field is required</Text>
      )}
    </View>
  );
}
  ...
  return (
    <View style={timerS.display}>
      <Animated.View style={{
        ...
        <Text style={[timerS.digits, { color }]}>
          {fmt(h)}:{fmt(m)}:{fmt(s)}
        </Text>
      </Animated.View>
      ...
      <View style={pmS.overlay}>
        <Animated.View style={[pmS.sheet, { transform: [{ translateY: slideY }], opacity }]}>
          ...
          <Text style={pmS.body}>
            ...
            your labor time won't be fully captured, which can affect work order
            accuracy and your audit trail.
          </Text>
...
const pmS = StyleSheet.create({
  ...
  sheet: {
    ...
    borderWidth: 1, borderColor: HUD.amber + '60', padding: 24,
    paddingBottom: 44, position: 'relative',
    shadowColor: HUD.amber, shadowOffset: { width: 0, height: -4 },
  ...
  return (
    ...
    >
      ...
      <Text style={[clS.label, { color: checked ? HUD.text : HUD.textSec }]}>
        {item.label}
        {item.required && <Text style={{ color: HUD.red }}> *</Text>}
      </Text>
    </TouchableOpacity>
  );
}
const clS = StyleSheet.create({
  row: {
    ...
    backgroundColor: HUD.bgCardAlt, borderRadius: 8, borderWidth: 1, padding: 11, marginBottom: 6,
  },
...
export default function SanitationWorkOrderDetail({ workOrder: initialWO, onClose, canEdit = true }: Props) {
  ...
  useEffect(() => {
    if (timerRunning) {
      timerRef.current = setInterval(() => {
        ...
        const since = timerStartRef.current
          ? Math.floor((Date.now() - timerStartRef.current.getTime()) / 1000)
          : 0;
  ...
  const handlePauseLeave = useCallback(async () => {
    ...
    const addSec = timerStartRef.current
      ? Math.floor((Date.now() - timerStartRef.current.getTime()) / 1000)
      : 0;
    try {
      const updated = await pauseTimer({ id: wo.id, additionalSec: addSec });
      setWO(updated);
    } catch (e) {
      console.error('[SanWO] pauseTimer failed', e);
    }
    onClose();
  }, [wo.id, timerRunning]);
  ...
  const isQAUser = user?.department_id === 1004; // Quality dept

  // ── ATP result calc ───────────────────────────────────────────────────────────
  const atpRLU = parseFloat(atpReading);
  const atpResult = !isNaN(atpRLU)
    ? atpRLU <= (wo.atp_pass_threshold || 250) ? 'pass' : 'fail'
    : null;
  ...
  const handleStartTimer = useCallback(async () => {
    ...
    try {
      ...
      console.error('[SanWO] startTimer failed', e);
      setWO(prev => ({ ...prev, timer_started_at: null }));
    }
  }, [wo.id, wo.started_at]);
  ...
  const handleAddPhoto = useCallback(async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Camera Access Required', 'Please allow camera access in Settings to capture task photos.');
      return;
    }
    const result = await ImagePicker.launchCameraAsync({ quality: 0.8, allowsEditing: false });
    if (!result.canceled && result.assets[0]) {
      const photo: SanPhoto = {
  ...
  const handleRemovePhoto = useCallback(async (index: number) => {
    Alert.alert('Remove Photo', 'Remove this photo from the task record?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          const updated = await removePhoto({ id: wo.id, photoIndex: index });
          setWO(updated);
        },
      },
    ]);
  ...
  const allRequiredChecked = (wo.checklist_items || [])
    .filter(i => i.required)
    .every(i => checklistCompletion[i.id]);
  ...
  // ── Save field changes periodically ──────────────────────────────────────────
  const saveRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const autoSave = useCallback((field: string, value: any) => {
    if (saveRef.current) clearTimeout(saveRef.current);
    saveRef.current = setTimeout(async () => {
      try {
        await updateWorkOrder({ id: wo.id, updates: { [field]: value } });
      } catch (e) {
        console.error('[SanWO] autoSave failed', e);
      }
    }, 1500);
  }, [wo.id]);
  ...
  const handleSubmitTech = useCallback(async () => {
    if (!canSubmitTech) {
      Alert.alert('Incomplete Task', 'All required fields must be filled. Every checklist item marked required must be checked. At least one photo required. Initial and PPN required.');
      return;
    }
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    ...
    if (timerRunning && timerStartRef.current) {
      additionalSec = Math.floor((Date.now() - timerStartRef.current.getTime()) / 1000);
    }
  ...
  const handleSubmitQA = useCallback(async () => {
    if (!canSubmitQA) {
      Alert.alert('Incomplete QA Sign-Off', 'Tech signature must be complete before QA can sign. Initial and PPN are required.');
      return;
    }
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    try {
      const updated = await submitQASignature({
        ...
        initial: qaInitial,
        ppn: qaPPN,
        signedByName: user ? `${user.first_name} ${user.last_name}` : 'QA',
        signedById: user?.id || '',
      });
      setWO(updated);
    } catch (e) {
    ...
    <View style={s.container}>
      ...
      <View style={[s.header, { borderBottomColor: statusColor + '50', shadowColor: statusColor }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.woNumber}>{wo.sanitation_wo_number}</Text>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
            ...
            <HUDCard>
              <View style={s.infoGrid}>
                <View style={s.infoCell}>
                  ...
                  <Text style={s.infoValue}>{wo.room_code || wo.area || '—'}</Text>
                </View>
                <View style={s.infoCell}>
            ...
            <HUDCard color={HUD.green + '40'}>
              ...
              <View style={s.assignedRow}>
                ...
                <Text style={[s.assignedName, { color: HUD.green }]}>
                  {wo.assigned_to_name || (user ? `${user.first_name} ${user.last_name}` : 'Unassigned')}
                </Text>
          ...
          <View style={s.section}>
            ...
            <HUDCard color={timerColor + '50'} style={{ alignItems: 'center' }}>
              ...
              {isEditable && !isCompleted && !isAwaitingQA && (
                <View style={s.timerBtnRow}>
                  {!timerRunning ? (
                    <TouchableOpacity
                      activeOpacity={0.75}
                      style={[s.timerBtn, { backgroundColor: HUD.cyanDim, borderColor: HUD.cyanMid }]}
                      onPress={handleStartTimer}
                    >
                      <Play size={15} color={HUD.cyan} />
                      <Text style={[s.timerBtnTxt, { color: HUD.cyan }]}>
                        {elapsed > 0 ? 'RESUME TASK' : 'START TASK'}
                      </Text>
                    </TouchableOpacity>
                  ) : (
                    <TouchableOpacity
                      activeOpacity={0.75}
                      style={[s.timerBtn, { backgroundColor: HUD.amber + '18', borderColor: HUD.amber + '50' }]}
              ...
              {(isCompleted || isAwaitingQA) && elapsed > 0 && (
                <Text style={s.finalTime}>
                  TOTAL LABOR: {formatElapsed(elapsed)}
                </Text>
              )}
            </HUDCard>
          </View>
          ...
          <View style={s.section}>
            ...
            <HUDCard>
              <HUDInput
                label="CHEMICAL USED"
                value={chemical}
                onChangeText={(t) => { setChemical(t); autoSave('chemical_used', t); }}
                required
                editable={isEditable && !techSigned}
                placeholder="e.g. Kay-5 Sanitizer"
              />
              <HUDInput
                label="ACTUAL CONCENTRATION (PPM / RATIO)"
                value={concentration}
                onChangeText={(t) => { setConcentration(t); autoSave('actual_concentration', t); }}
                required
                editable={isEditable && !techSigned}
                placeholder="e.g. 200 ppm / 1:128"
              />
              <HUDInput
                label="APPLICATION METHOD"
                value={method}
                onChangeText={(t) => { setMethod(t); autoSave('application_method', t); }}
                required
                editable={isEditable && !techSigned}
                placeholder="e.g. Spray & Wipe, Foam, Flood"
              />
            </HUDCard>
          </View>

          {/* ── SECTION 4b: CHECKLIST ── */}
              ...
              <View>
                {(wo.checklist_items || []).map((item) => (
                  <ChecklistRow
                    key={item.id}
                    item={item}
                    checked={!!checklistCompletion[item.id]}
                    onToggle={() => {
                      if (!isEditable || techSigned) return;
          ...
          {/* ── SECTION 5: ATP READING (conditional) ── */}
          {wo.requires_atp_test && (
            <View style={s.section}>
              ...
              <HUDCard color={HUD.amber + '40'}>
                <HUDInput
                  label="RLU READING"
                  value={atpReading}
                  onChangeText={setAtpReading}
                  required
                  keyboardType="numeric"
                  editable={isEditable && !techSigned}
                  accentColor={HUD.amber}
                  placeholder="Enter RLU value from meter"
                />
                {atpResult && (
                  <View style={[s.atpResult, {
                    backgroundColor: atpResult === 'pass' ? HUD.green + '15' : HUD.red + '15',
                    ...
                    <Text style={[s.atpResultTxt, { color: atpResult === 'pass' ? HUD.green : HUD.red }]}>
                      {atpResult === 'pass'
                        ? `PASS — ${atpRLU} RLU ≤ ${wo.atp_pass_threshold} threshold`
                        : `FAIL — ${atpRLU} RLU exceeds ${wo.atp_pass_threshold} threshold`}
                    </Text>
                  </View>
                )}
                {atpResult === 'fail' && (
                  <View style={[s.alertBanner, { borderColor: HUD.red + '50', backgroundColor: HUD.redDim, marginTop: 8 }]}>
                    ...
                    <Text style={[s.alertTxt, { color: HUD.red }]}>
                      ATP failure requires corrective action. Post to task feed and notify Quality.
                    </Text>
                  </View>
                )}
              </HUDCard>
          ...
          <View style={s.section}>
            <HUDCard>
              <HUDInput
                label="NOTES"
                value={notes}
                onChangeText={(t) => { setNotes(t); autoSave('notes', t); }}
                editable={isEditable}
                multiline
                ...
                <View style={s.signedConfirm}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.signedMeta}>
                      {wo.tech_initial} · PPN {wo.tech_ppn} · {wo.tech_signed_at ? new Date(wo.tech_signed_at).toLocaleString() : ''}
                    </Text>
                  </View>
                </View>
              ) : (
                  ...
                  <View style={s.sigRow}>
                    ...
                    <View style={{ flex: 1 }}>
                      <HUDInput
                        label="PPN"
                        value={techPPN}
                        onChangeText={setTechPPN}
                        required
                        maxLength={10}
                        editable={isEditable && !techSigned}
                        placeholder="Employee PPN"
                      />
                    </View>
                  </View>
                  {isEditable && (
                    ...
                    >
                      {isSubmittingTech
                        ...
                        : <CheckCircle size={16} color={HUD.cyan} />}
          ...
          {/* ── SECTION 9: QA SIGNATURE (conditional) ── */}
          {wo.requires_qa_signoff && (
            <View style={s.section}>
                ...
                {wo.qa_signed_at ? (
                  <View style={s.signedConfirm}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={s.signedMeta}>
                        {wo.qa_initial} · PPN {wo.qa_ppn} · {wo.qa_signed_by_name} · {wo.qa_signed_at ? new Date(wo.qa_signed_at).toLocaleString() : ''}
                      </Text>
                    </View>
                  </View>
                ) : !techSigned ? (
                  ...
                  <>
                    ...
                    >
                      {isSubmittingQA
                        ...
                        : <CheckCircle size={16} color={HUD.amber} />}
          ...
          {isCompleted && (
            <HUDCard color={HUD.green + '60'} style={{ alignItems: 'center', gap: 8, marginTop: 8 }}>
              ...
              {wo.completed_at && (
                <Text style={{ fontSize: 11, color: HUD.textSec }}>
      ...
      <PauseModal
        ...
        onPauseLeave={handlePauseLeave}
        onStay={() => setShowPauseModal(false)}
      />
    </View>
  );
}
