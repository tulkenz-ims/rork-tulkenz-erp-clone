    ...
    queryFn: async () => {
      ...
      return (data || []).map((row: any) => ({
        ...
        departmentTasks: (row.task_feed_department_tasks || []).map((t: any) => ({
          ...
          moduleHistoryType: t.module_reference_type || t.module_history_type,
          moduleHistoryId: t.module_reference_id || t.module_history_id,
        })),
      }));
    },
    enabled: options?.enabled !== false && !!organizationId,
...
export function useCreateTaskFeedPost(callbacks?: MutationCallbacks<TaskFeedPost & { totalDepartments: number }>) {
  ...
  return useMutation({
    mutationFn: async (input: CreatePostInput) => {
      ...
      // Production hold: true if template says so OR if user answered "Yes" to production stopped
      const isProductionHold = template.is_production_hold || input.productionStopped || false;
      const productionLine = input.roomLine || productionLineFromForm || null;
