...
const TOOLS = [
  ...
  { name: 'create_task_feed_post_chemical_spill', description: 'Report a chemical spill.', input_schema: { type: 'object', properties: { location: { type: 'string' }, chemical_name: { type: 'string' }, quantity_spilled: { type: 'string' }, product_contact: { type: 'string', enum: ['Yes', 'No', 'Unknown'] }, immediate_action_taken: { type: 'string' }, area_cleared: { type: 'boolean' }, additional_notes: { type: 'string' } }, required: ['location', 'chemical_name', 'quantity_spilled', 'product_contact', 'immediate_action_taken', 'area_cleared'] } },
  { name: 'create_task_feed_post_employee_injury', description: 'Report an employee injury or near miss.', input_schema: { type: 'object', properties: { location: { type: 'string' }, injury_type: { type: 'string', enum: ['Cut', 'Burn', 'Strain/Sprain', 'Slip/Fall', 'Chemical Exposure', 'Near Miss', 'Other'] }, body_part: { type: 'string' }, employee_name: { type: 'string' }, description: { type: 'string' }, medical_attention_required: { type: 'boolean' }, immediate_action_taken: { type: 'string' }, additional_notes: { type: 'string' } }, required: ['location', 'injury_type', 'body_part', 'employee_name', 'description', 'medical_attention_required', 'immediate_action_taken'] } },
  { name: 'create_task_feed_post_equipment_breakdown', description: 'Report equipment breakdown.', input_schema: { type: 'object', properties: { location: { type: 'string' }, equipment_name: { type: 'string' }, symptom: { type: 'string' }, production_impact: { type: 'string', enum: ['Line Down', 'Reduced Speed', 'No Impact', 'Unknown'] }, immediate_action_taken: { type: 'string' }, create_work_order_requested: { type: 'boolean' }, additional_notes: { type: 'string' } }, required: ['location', 'equipment_name', 'symptom', 'production_impact', 'immediate_action_taken', 'create_work_order_requested'] } },
  { name: 'create_task_feed_post_metal_detector_reject', description: 'Report a metal detector reject.', input_schema: { type: 'object', properties: { location: { type: 'string' }, product_affected: { type: 'string' }, quantity_rejected: { type: 'string' }, metal_found: { type: 'string', enum: ['Yes', 'No', 'Under Investigation'] }, production_line: { type: 'string', enum: ['Line 1', 'Line 2', 'Line 3', 'N/A'] }, immediate_action_taken: { type: 'string' }, additional_notes: { type: 'string' } }, required: ['location', 'product_affected', 'quantity_rejected', 'metal_found', 'production_line', 'immediate_action_taken'] } },
  { name: 'create_task_feed_post_pest_sighting', description: 'Report a pest sighting.', input_schema: { type: 'object', properties: { location: { type: 'string' }, pest_type: { type: 'string', enum: ['Rodent', 'Cockroach', 'Fly', 'Ant', 'Bird', 'Other Insect', 'Other'] }, number_observed: { type: 'string' }, product_contact: { type: 'string', enum: ['Yes', 'No', 'Unknown'] }, evidence_type: { type: 'string', enum: ['Live', 'Dead', 'Droppings', 'Damage', 'Tracks', 'Other'] }, immediate_action_taken: { type: 'string' }, additional_notes: { type: 'string' } }, required: ['location', 'pest_type', 'number_observed', 'product_contact', 'evidence_type', 'immediate_action_taken'] } },
  { name: 'create_task_feed_post_temperature_deviation', description: 'Report a temperature deviation.', input_schema: { type: 'object', properties: { location: { type: 'string' }, recorded_temp: { type: 'string' }, required_temp: { type: 'string' }, duration: { type: 'string' }, product_affected: { type: 'string' }, immediate_action_taken: { type: 'string' }, additional_notes: { type: 'string' } }, required: ['location', 'recorded_temp', 'required_temp', 'duration', 'product_affected', 'immediate_action_taken'] } },
  { name: 'create_task_feed_post_customer_complaint', description: 'Log a customer complaint.', input_schema: { type: 'object', properties: { complaint_type: { type: 'string', enum: ['Foreign Material', 'Off Flavor/Odor', 'Underfill', 'Seal Failure', 'Label Issue', 'Allergen Concern', 'Other'] }, product_name: { type: 'string' }, lot_number: { type: 'string' }, description: { type: 'string' }, customer_name: { type: 'string' }, immediate_action_taken: { type: 'string' }, additional_notes: { type: 'string' } }, required: ['complaint_type', 'product_name', 'lot_number', 'description', 'customer_name', 'immediate_action_taken'] } },
  { name: 'create_task_feed_post_generic', description: 'Create a generic task feed post when no specific template matches.', input_schema: { type: 'object', properties: { post_type: { type: 'string', enum: ['report_issue', 'create_task', 'purchase_request', 'pre_op', 'other'] }, template_name: { type: 'string' }, departments: { type: 'array', items: { type: 'string' } }, location: { type: 'string' }, notes: { type: 'string' }, priority: { type: 'string', enum: ['low', 'medium', 'high', 'critical'] } }, required: ['post_type', 'template_name', 'departments', 'notes', 'priority'] } },
  { name: 'query_task_feed', description: 'Search task feed posts and work orders. For PMs set post_type="preventive".', input_schema: { type: 'object', properties: { department_code: { type: 'string' }, status: { type: 'string', enum: ['pending', 'in_progress', 'completed', 'all'] }, post_type: { type: 'string' }, date_range: { type: 'string', enum: ['today', 'this_week', 'this_month', 'all'] } }, required: ['status'] } },
  { name: 'query_records', description: 'Search and list records from ANY module or table in the app.', input_schema: { type: 'object', properties: { table: { type: 'string' }, filters: { type: 'object', additionalProperties: true }, search_column: { type: 'string' }, search_term: { type: 'string' }, order_by: { type: 'string' }, order_direction: { type: 'string', enum: ['asc', 'desc'] }, date_filter_column: { type: 'string' }, date_range: { type: 'string', enum: ['today', 'this_week', 'this_month', 'this_year', 'all'] }, limit: { type: 'number' } }, required: ['table'] } },
  { name: 'create_work_order', description: 'Create a maintenance work order for repairs. Do NOT use for reporting incidents.', input_schema: { type: 'object', properties: { title: { type: 'string' }, equipment_name: { type: 'string' }, description: { type: 'string' }, priority: { type: 'string', enum: ['low', 'medium', 'high', 'critical'] }, type: { type: 'string', enum: ['reactive', 'preventive', 'predictive'] }, parts_needed: { type: 'array', items: { type: 'string' } } }, required: ['title', 'equipment_name', 'description', 'priority', 'type'] } },
  { name: 'start_pre_op', description: 'Start a Pre-Op inspection for a room.', input_schema: { type: 'object', properties: { room: { type: 'string', enum: ['PR1', 'PR2', 'PA1', 'PA2', 'BB1', 'SB1', 'PW1', 'PO1', 'PO2', 'PO3', 'WB1'] } }, required: ['room'] } },
  { name: 'lookup_part', description: 'Search Item Records by name, part number, SKU, vendor part number, or manufacturer part number. Always try this first before web search.', input_schema: { type: 'object', properties: { query: { type: 'string' } }, required: ['query'] } },
  { name: 'lookup_equipment', description: 'Get equipment details or troubleshooting info.', input_schema: { type: 'object', properties: { query: { type: 'string' }, section: { type: 'string', enum: ['specs', 'manual', 'troubleshooting', 'pm_history', 'parts'] } }, required: ['query'] } },
  { name: 'diagnose_issue', description: 'Analyze equipment problem from description or photo.', input_schema: { type: 'object', properties: { equipment: { type: 'string' }, symptom: { type: 'string' }, create_work_order: { type: 'boolean' } }, required: ['equipment', 'symptom', 'create_work_order'] } },
...
async function loadMemory(orgId, userId) {
  try {
    ...
    const { data: memories } = await sb.from('ai_assistant_memory').select('category,key,value,context,times_confirmed').eq('organization_id', orgId).or(`user_id.eq.${userId},user_id.is.null`).order('times_confirmed', { ascending: false }).order('last_seen_at', { ascending: false }).limit(20);
    const { data: summaries } = await sb.from('ai_conversation_summaries').select('summary,topics,actions_taken,unresolved,created_at').eq('organization_id', orgId).eq('user_id', userId).order('created_at', { ascending: false }).limit(3);
...
function buildMemoryBlock(memories, summaries) {
  ...
  if (memories.length > 0) {
    ...
    Object.entries(byCategory).forEach(([cat, facts]) => { lines.push(`${cat.toUpperCase()}: ${facts.join(' | ')}`); });
    lines.push('');
  }
...
async function extractAndSaveMemory(orgId, userId, userName, conversation, sb) {
  ...
  try {
    ...
    const convoText = conversation.map(m => `${m.role.toUpperCase()}: ${typeof m.content === 'string' ? m.content : JSON.stringify(m.content)}`).join('\n');
    const extractResp = await client.messages.create({
      model: 'claude-haiku-4-5-20251001', max_tokens: 600,
      messages: [{ role: 'user', content: `You are a memory extractor for an AI assistant in a food manufacturing facility. Extract from this conversation:\n1. A 2-3 sentence SUMMARY\n2. Facts to remember about this user\n3. Language used\n\nConversation:\n${convoText}\n\nRespond ONLY with valid JSON:\n{\n  "summary": "...",\n  "topics": ["topic1"],\n  "actions_taken": ["action1"],\n  "unresolved": "anything left open or null",\n  "memories": [\n    { "category": "preference|part|equipment|shorthand|workflow|person|language", "key": "short_key", "value": "value", "context": "optional or null" }\n  ]\n}` }],
    });
    const raw = extractResp.content[0]?.text || '';
    const clean = raw.replace(/```json|```/g, '').trim();
    const parsed = JSON.parse(clean);
    if (parsed.summary) {
      await sb.from('ai_conversation_summaries').insert({ organization_id: orgId, user_id: userId, user_name: userName, summary: parsed.summary, topics: parsed.topics || [], actions_taken: parsed.actions_taken || [], unresolved: parsed.unresolved || null, message_count: conversation.length });
    }
    if (parsed.memories?.length > 0) {
      for (const mem of parsed.memories) {
        if (!mem.key || !mem.value) continue;
        const { data: existing } = await sb.from('ai_assistant_memory').select('id,times_confirmed').eq('organization_id', orgId).eq('user_id', userId).eq('category', mem.category).eq('key', mem.key).maybeSingle();
        if (existing) {
          await sb.from('ai_assistant_memory').update({ value: mem.value, context: mem.context, times_confirmed: (existing.times_confirmed || 1) + 1, last_seen_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', existing.id);
        } else {
          await sb.from('ai_assistant_memory').insert({ organization_id: orgId, user_id: userId, category: mem.category, key: mem.key, value: mem.value, context: mem.context || null });
        }
      }
    }
  } catch (e) {
...
const DEFAULT_CAPS = { platform_admin: 0, superadmin: 0, super_admin: 0, administrator: 0, admin: 0, manager: 200, employee: 50, default: 50 };
...
async function checkRateLimit(sb, orgId, userId, userRole, isPlatformAdmin) {
  ...
  try {
    const { data: empLimit } = await sb.from('ai_rate_limits').select('daily_limit, is_unlimited').eq('organization_id', orgId).eq('employee_id', userId).maybeSingle();
    let limit = null;
    if (empLimit) {
    ...
    if (limit === null) {
      const { data: roleLimit } = await sb.from('ai_rate_limits').select('daily_limit, is_unlimited').eq('organization_id', orgId).eq('role_name', userRole || 'default').is('employee_id', null).maybeSingle();
      if (roleLimit) {
    ...
    const { count } = await sb.from('ai_usage_log').select('id', { count: 'exact', head: true }).eq('organization_id', orgId).eq('employee_id', userId).eq('is_platform_admin', false).gte('created_at', todayStart.toISOString());
    const used = count || 0;
...
async function logUsage(sb, { orgId, userId, userName, userRole, deptCode, screen, toolUsed, commandPreview, usage, model, language, hadImage, hadWebSearch, loopCount, responseMs, isPlatformAdmin }) {
  if (!sb || !orgId) return;
  try {
...
async function checkWatchList(sb, orgId, userId) {
  ...
  try {
    const { data } = await sb.from('ai_watch_list').select('id, is_active, email_alerts, alert_email').eq('organization_id', orgId).eq('employee_id', userId).eq('is_active', true).maybeSingle();
    return data || null;
  } catch (e) {
...
async function captureWatchedConversation(sb, orgId, watchEntry, userId, userName, conversation, speechText, command, includeFullTranscript = false) {
  ...
  try {
    ...
    const extractResp = await client.messages.create({ model: 'claude-haiku-4-5-20251001', max_tokens: 400, messages: [{ role: 'user', content: `Summarize this AI assistant conversation from a food manufacturing facility.\nConversation:\n${convoText}\nRespond ONLY with valid JSON:\n{"summary":"2-3 sentence summary","topics":["topic1"],"actions_taken":["action1"],"unresolved":"anything unresolved or null"}` }] });
    const raw = extractResp.content[0]?.text || '';
    const clean = raw.replace(/```json|```/g, '').trim();
...
async function saveUserNote(sb, orgId, userId, userName, noteType, content, title, reminderAt) {
  if (!sb || !orgId || !userId) return null;
  try {
    const { data, error } = await sb.from('ai_user_notes').insert({ organization_id: orgId, employee_id: userId, employee_name: userName, note_type: noteType || 'note', title: title || null, content, reminder_at: reminderAt || null, source: 'ai_assistant' }).select('id').single();
...
async function saveConversationRecord(sb, orgId, userId, userName, deptCode, deptName, conversation, speechText, command) {
  ...
  try {
    ...
    const client = new Anthropic({ apiKey });
    const convoText = conversation.map(m => `${m.role.toUpperCase()}: ${typeof m.content === 'string' ? m.content : JSON.stringify(m.content)}`).join('\n') + `\nUSER: ${command}\nASSISTANT: ${speechText}`;
    const extractResp = await client.messages.create({ model: 'claude-haiku-4-5-20251001', max_tokens: 400, messages: [{ role: 'user', content: `Summarize this AI assistant conversation briefly.\nConversation:\n${convoText}\nRespond ONLY with valid JSON:\n{"summary":"2-3 sentence summary","topics":["topic1"],"actions_taken":["action1"],"unresolved":"anything unresolved or null"}` }] });
    const raw = extractResp.content[0]?.text || '';
    const clean = raw.replace(/```json|```/g, '').trim();
    let parsed = { summary: speechText, topics: [], actions_taken: [], unresolved: null };
    try { parsed = JSON.parse(clean); } catch (e) {}
    const { data, error } = await sb.from('ai_saved_conversations').insert({ organization_id: orgId, employee_id: userId, employee_name: userName, department_code: deptCode || null, department_name: deptName || null, conversation_id: `conv-${Date.now()}-${Math.random().toString(36).slice(2)}`, summary: parsed.summary || null, topics: parsed.topics || [], actions_taken: parsed.actions_taken || [], unresolved: parsed.unresolved || null, message_count: conversation.length + 1, saved_by: 'user', full_transcript: conversation }).select('id').single();
    if (error) throw error;
    return data;
  } catch (e) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    while (loopCount < MAX_LOOPS) {
      loopCount++;
      response = await client.messages.create({ model: 'claude-sonnet-4-20250514', max_tokens: 1024, system: dynamicSystem, tools: TOOLS, messages });
      ...
      const toolResults = response.content.filter(b => b.type === 'server_tool_use').map(b => ({ type: 'tool_result', tool_use_id: b.id, content: b.output || '' }));
    ...
    const result = {
      ...
      rate_limit: rateCheck.warn ? { used: rateCheck.used, limit: rateCheck.limit, remaining: rateCheck.remaining, warn: true } : null,
    };
    ...
    if (result.tool_name && noteTools.includes(result.tool_name)) {
      if (result.tool_name === 'save_note') {
        const saved = await saveUserNote(sb, orgId, userId, userName, 'note', result.params?.content || '', result.params?.title || null, null);
        result.action = 'info';
        result.speech = saved ? (es ? `Nota guardada: "${result.params?.title || result.params?.content}"` : `Note saved: "${result.params?.title || result.params?.content}"`) : (es ? 'No pude guardar la nota.' : 'Could not save the note.');
      }
      ...
      if (result.tool_name === 'save_conversation') {
        setImmediate(async () => { await saveConversationRecord(sb, orgId, userId, userName, context?.userDepartment || null, null, conversation || [], result.speech || '', command || ''); });
        result.action = 'info';
        result.speech = es ? 'Conversación guardada en tu historial personal.' : 'Conversation saved to your personal log.';
      }
    }
