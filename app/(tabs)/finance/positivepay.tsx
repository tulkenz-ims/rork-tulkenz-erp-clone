...
import { Send, ArrowLeft, Shield, FileText, AlertTriangle } from 'lucide-react-native';
import { useTheme } from '@/contexts/ThemeContext';

export default function PositivePayScreen() {
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.content}>
        <View style={[styles.placeholderCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={[styles.iconContainer, { backgroundColor: '#991B1B15' }]}>
            <Send size={48} color="#991B1B" />
          </View>
          <Text style={[styles.title, { color: colors.text }]}>Positive Pay Files</Text>
          <Text style={[styles.description, { color: colors.textSecondary }]}>
            Generate positive pay files to prevent check fraud with bank integration.
          </Text>
          
          <View style={styles.featureList}>
            <View style={styles.featureItem}>
              <Shield size={18} color="#991B1B" />
              <Text style={[styles.featureText, { color: colors.text }]}>Fraud Prevention</Text>
            </View>
            <View style={styles.featureItem}>
              <FileText size={18} color="#991B1B" />
              <Text style={[styles.featureText, { color: colors.text }]}>Bank File Generation</Text>
            </View>
            <View style={styles.featureItem}>
              <AlertTriangle size={18} color="#991B1B" />
              <Text style={[styles.featureText, { color: colors.text }]}>Exception Handling</Text>
            </View>
          </View>

          <View style={[styles.comingSoonBadge, { backgroundColor: '#991B1B15' }]}>
            <Text style={[styles.comingSoonText, { color: '#991B1B' }]}>Coming Soon</Text>
          </View>
        </View>
