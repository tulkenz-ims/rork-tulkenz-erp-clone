    ...
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      <View style={s.heroCenter}>
        ...
        <Text style={[s.heroDesc, { marginTop: 12 }]}>
          Purpose-built operations software for food manufacturing.{'\n'}
...
function Slide02() {
  return (
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.threeCol}>
        ...
        <Card accent={T.cyan} style={{ flex: 1 }}>
          ...
          {[
            ...
            'Sanitation Scheduling & Verification',
            'Safety / OSHA Compliance',
            'Task Feed + Push Routing',
            'IoT Sensors & Andon',
            'AI-Assisted Operations',
            'Auditor Portal (SQF/FDA/OSHA/ESG)',
      ...
      <Card accent={T.teal} style={{ marginTop: 8 }}>
        <Text style={[s.smallText, { color: T.teal }]}>
          Integration capable: NetSuite PO sync, Rippling employee roster sync — bidirectional, REST API, webhook events for WO status changes.
        </Text>
      </Card>
    </ScrollView>
  );
...
function Slide03() {
  const groups = [
    {
      label: 'Operations Core', color: T.cyan,
      modules: ['Task Feed (hub for all activity)', 'Scheduled Tasks & PM Auto-Post', 'Issue Reporting w/ Templates', 'Purchase Requests', 'Work Orders & Equipment'],
    },
    {
      label: 'Quality & Compliance', color: T.teal,
      modules: ['NCR / CAPA', 'Auditor Portal (9 frameworks)', 'Pre-Op Inspections', 'HACCP / SQF Documentation', 'SDS Manager', 'Policies, Procedures & SOPs', 'Document Control', 'Internal Audit Management'],
    },
    {
      label: 'Sanitation', color: T.blue,
      modules: ['Master Sanitation Schedule', 'Sanitation Work Orders', 'Pre-Op Inspections', 'ATP Swab Records', 'SSOP Library', 'Sanitation CAPA'],
    },
    {
      label: 'Safety & People', color: T.amber,
      modules: ['OSHA Permits & Compliance', 'Incident Tracking', 'Training Records & Certs', 'Emergency Protocol System', 'Employee Check-In / Check-Out', 'Room Time Tracking'],
    },
    {
      label: 'Procurement', color: T.purple,
      modules: ['Purchase Orders (non-production)', 'Parts & MRO Inventory', 'Vendor Approvals', 'Receiving', 'Parts Catalog', 'Expandable to full supply chain'],
    },
    {
      label: 'Intelligence', color: T.red,
      modules: ['AI Assistant (Claude)', 'IoT Room Sensors', 'Andon Light System', 'Push Notifications', 'Analytics & Reporting'],
    },
  ];
  return (
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
...
function Slide04() {
  return (
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      <SlideHeader
        title="Task Feed — The Operational Hub"
        subtitle="Every operation flows through Task Feed — scheduled, reactive, or requested"
      />
      <View style={s.twoCol}>
        <Card accent={T.cyan} style={{ flex: 1 }}>
          <Text style={[s.cardTitle, { color: T.cyan }]}>What Flows Through Task Feed</Text>
          {[
            'Scheduled sanitation & maintenance tasks',
            'PM auto-post — PM schedule fires → Task Feed post created automatically',
            'Issue reports with common issue templates (equipment down, broken glove, spill, etc.)',
            'Purchase requests routed to procurement',
            'Task Feed reverses too — WO completion posts back to feed',
            'Cross-department notifications with push alerts',
            'Required fields enforce complete data capture',
            'Every post creates a timestamped audit trail',
          ].map(t => <BulletRow key={t} text={t} color={T.cyan} />)}
        </Card>
        <View style={{ flex: 1, gap: 10 }}>
          <Card accent={T.purple}>
            <Text style={[s.cardTitle, { color: T.purple }]}>Auto-Routing Logic</Text>
            {[
              'Issue type determines target department',
              'Push notification sent immediately',
              'Work order created automatically',
              'PPN signature required on completion',
              'Full event chain preserved',
            ].map(t => <BulletRow key={t} text={t} color={T.purple} />)}
          </Card>
          <Card accent={T.amber}>
            <Text style={[s.cardTitle, { color: T.amber }]}>What Makes It Different</Text>
            {[
              'Not just a ticket system — it is the operational backbone',
              'Scheduled tasks, reactive issues, and requests in one feed',
              'Bidirectional — actions trigger and complete in the feed',
              'No other platform connects all of this in one view',
            ].map(t => <BulletRow key={t} text={t} color={T.amber} />)}
          </Card>
        </View>
      </View>
    </ScrollView>
      ...
      <View style={s.twoCol}>
        <Card accent={T.cyan} style={{ flex: 1.1 }}>
          ...
          {[
            ...
            'File work orders from a text or voice command',
        ...
        <View style={{ flex: 1, gap: 10 }}>
          <Card accent={T.purple}>
            ...
            {[
              'Anthropic Tool Use API (function calling)',
              'JSON-structured actions mapped to app hooks',
              'Full conversation context preserved per session',
              'Org-scoped — AI only sees your data',
            ].map(t => <BulletRow key={t} text={t} color={T.purple} />)}
          </Card>
          <Card accent={T.teal}>
            <Text style={[s.cardTitle, { color: T.teal }]}>Active Development</Text>
            {[
              'Auto-open web pages for part lookups',
              'Spanish language responses (output)',
              'Predictive maintenance from sensor data',
              'Auto-generate audit evidence packages',
              'Voice command integration',
            ].map(t => <BulletRow key={t} text={t} color={T.teal} />)}
          </Card>
        </View>
      </View>
      <Quote text="Not a chatbot bolted on — Claude is wired into every module's data layer and takes real actions." color={T.cyan} />
    </ScrollView>
  );
}
    ...
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.threeCol}>
        <Card accent={T.teal} style={{ flex: 1 }}>
          ...
          {[
            'Temperature monitoring per room',
            'Humidity & air quality sensors',
            'Simulated now — ESP32 hardware ready',
            'Real-time alerts to task feed',
        ...
        <Card accent={T.red} style={{ flex: 1 }}>
          ...
          {[
            ...
            'All-clear confirmation workflow',
            'Logged & timestamped for OSHA',
          ].map(t => <BulletRow key={t} text={t} color={T.red} />)}
        </Card>
      </View>
      <Card accent={T.blue} style={{ marginTop: 10 }}>
        ...
        <Text style={s.smallText}>LED Andon bars above each room entrance — software already live, hardware is plug-and-play when ready</Text>
        <Text style={[s.smallText, { color: T.gray, marginTop: 4 }]}>Current: Fully simulated in-app  •  Next: Physical sensors wired to same API endpoints — no code changes required</Text>
      </Card>
...
function Slide07() {
  const items = [
    ...
    { title: 'Token-Based Audit Access', desc: 'Auditor Portal uses SHA-256 hashed time-limited tokens. Read-only, revocable, all access logged — no login credentials ever shared.', color: T.purple },
...
function Slide08() {
  const packages = [
    ...
    },
    {
      ...
      items: [
        'SQF / BRCGS / FSSC / FDA audit-ready exports',
    ...
    },
    {
      ...
      items: [
        'REST API access to all modules',
        'NetSuite PO sync capability',
        'Rippling employee data sync capability',
        'Webhook events for WO status',
  ...
  return (
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      <SlideHeader
        ...
        subtitle="Your data, your way — complete portability and integration capability at every tier"
      />
      <View style={s.threeCol}>
        {packages.map(pkg => (
      ...
      <Card accent={T.amber} style={{ marginTop: 10 }}>
        <Text style={[s.smallText, { color: T.amber, textAlign: 'center' }]}>
          Integration-capable: NetSuite PO sync  •  Rippling employee roster sync  •  Bidirectional REST API  •  Webhook event support
        </Text>
      </Card>
    </ScrollView>
  );
...
function Slide09() {
  const frameworks = [
    { name: 'SQF Ed. 10', color: T.cyan },
    { name: 'BRCGS Issue 9', color: T.teal },
    { name: 'FSSC 22000', color: T.blue },
    { name: 'FDA / FSMA', color: T.red },
    { name: 'OSHA 29 CFR', color: T.amber },
    { name: 'ESG / GRI', color: T.green },
    { name: 'Internal Audit', color: T.purple },
    { name: 'Regulatory', color: T.blue },
    ...
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      ...
      <Card accent={T.cyan} style={{ marginTop: 12 }}>
        ...
        <View style={s.twoCol}>
          <View style={{ flex: 1 }}>
            {[
              'Each framework renders its own nav structure',
              'Live data — SDS, NCRs, WOs, training, inspections',
              'PM Program tab + PM Records tab',
            ].map(t => <BulletRow key={t} text={t} color={T.cyan} />)}
          </View>
          <View style={{ flex: 1 }}>
            {[
              'Work Orders filtered to corrective & emergency',
              'Sanitation: 6 tabs (Schedule, Tasks, Pre-Op, ATP, SSOP, CAPA)',
...
function Slide10() {
  const phases = [
    {
      phase: 'PHASE 1', title: 'Discovery & Configuration', color: T.cyan,
      items: [
        'On-site process review — maintenance, quality, sanitation, safety workflows',
        'Map your existing processes to TulKenz modules',
        'Configure org structure, departments, rooms, equipment',
        'Import existing equipment lists, PMs, vendor records',
        'Set up employees, roles, and PPN signatures',
      ],
    },
    {
      phase: 'PHASE 2', title: 'Deployment & Training', color: T.teal,
      items: [
        'Department-by-department go-live training — on your floor, with your people',
        'CMMS activation: work orders, PMs, parts requests',
        'Task Feed activation: issue reporting, routing, notifications',
        'Quality & Sanitation module activation',
        'First audit portal walkthrough with your team',
      ],
    },
    {
      phase: 'PHASE 3', title: 'Optimization & Expansion', color: T.purple,
      items: [
        'Integration configuration (NetSuite, Rippling) as needed',
        'Custom report builds for your KPIs',
        'IoT sensor installation planning',
        'Module expansion based on operational priorities',
        'Documentation handoff + ongoing support agreement',
      ],
    },
  ];
  return (
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      <SlideHeader
        title="Implementation — What You're Getting"
      ...
      <Quote text="Nearly 30 years in the food service industry — I built this because I saw the gap from the inside." color={T.amber} />
    </ScrollView>
  );
}
...
function Slide11() {
  const rows = [
    ...
    { cap: 'Quality & Compliance Docs', ns: 'Basic document storage', tk: 'Policies, procedures, SOPs, NCR, CAPA, full doc control' },
    { cap: 'Task Feed / Routing', ns: 'Not available', tk: 'Scheduled tasks, issue templates, PM auto-post, bidirectional' },
    { cap: 'Mobile UX', ns: 'Poor — not mobile-native', tk: 'React Native — built mobile-first from day one' },
    { cap: 'AI Integration', ns: 'Add-on / third-party', tk: 'Claude embedded — takes real actions across all modules' },
    { cap: 'Auditor Access', ns: 'Report exports only', tk: 'Live portal, 9 frameworks, token-based read-only' },
    { cap: 'Onsite Implementation', ns: 'Certified partner required ($$$)', tk: 'Direct — built by the person implementing it' },
    { cap: 'Built for Food Mfg', ns: 'General ERP — adapt yourself', tk: 'Every module designed for food manufacturing operations' },
  ];
  return (
    <ScrollView style={s.slide} contentContainerStyle={s.slidePad} showsVerticalScrollIndicator={false}>
      <SlideHeader
...
function Slide12() {
  return (
    <ScrollView style={s.slide} contentContainerStyle={[s.slidePad, { alignItems: 'center' }]} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.threeCol}>
        {[
          { icon: '🤖', title: 'AI-Native', desc: 'Claude embedded in every module — not a chatbot, an operator that takes real actions', color: T.cyan },
          { icon: '🏭', title: 'Floor-Level', desc: 'Built from nearly 30 years in the food service industry — I know what breaks down and why', color: T.amber },
...
const SLIDES = [
  ...
  { id: '04', title: 'Task Feed', component: Slide04 },
  { id: '05', title: 'AI & Automation', component: Slide05 },
  { id: '06', title: 'IoT & Sensors', component: Slide06 },
