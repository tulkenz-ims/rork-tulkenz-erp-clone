...
export default function MonthlyTasksScreen() {
  ...
  const taskStats = useMemo(() => {
    const monthlyOnly = tasks.filter(t => t.task_type === 'monthly');
    return {
      total: monthlyOnly.length,
      completed: monthlyOnly.filter(t => t.status === 'completed').length,
      inProgress: monthlyOnly.filter(t => t.status === 'in_progress').length,
      pending: monthlyOnly.filter(t => t.status === 'pending').length,
    };
  }, [tasks]);
  ...
  const handleStartTask = useCallback(async (task: SanitationTask) => {
    ...
    try {
      ...
      console.error('[MonthlyTasks] Start error:', error);
  ...
  const handleCompleteTask = useCallback(async (task: SanitationTask) => {
    ...
    try {
      ...
      console.error('[MonthlyTasks] Complete error:', error);
  ...
  const handleAddTask = useCallback(async () => {
    ...
    const now = new Date();
    const scheduledDate = new Date(now.getFullYear(), now.getMonth(), selectedDayOfMonth);
    ...
    try {
      await createTask({
        ...
        facility_id: null,
        title: newTaskTitle,
        description: `Monthly task scheduled for day ${selectedDayOfMonth}`,
        instructions: null,
      ...
      setNewTaskLocation('');
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert('Success', 'Monthly task added successfully');
    } catch (error) {
      console.error('[MonthlyTasks] Add error:', error);
      Alert.alert('Error', 'Failed to add task');
    }
  }, [newTaskTitle, newTaskArea, newTaskLocation, selectedDayOfMonth, createTask, generateTaskNumber]);
      ...
      >
        <View style={styles.taskHeader}>
          ...
          <View style={styles.dateBadge}>
            ...
            <Text style={[styles.dateText, { color: '#6366F1' }]}>Day {dayOfMonth}</Text>
          </View>
        </View>
      ...
      >
        <View style={[styles.headerCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={[styles.iconContainer, { backgroundColor: '#6366F1' + '20' }]}>
            <CalendarRange size={28} color="#6366F1" />
          </View>
          <Text style={[styles.title, { color: colors.text }]}>Monthly Sanitation Tasks</Text>
            ...
            {(['all', 'pending', 'in_progress', 'completed'] as const).map((status) => (
              <Pressable
                ...
                style={[
                  ...
                  filterStatus === status && { backgroundColor: '#6366F1', borderColor: '#6366F1' },
                ]}
                onPress={() => {
        ...
        {monthlyTasks.length === 0 ? (
          <View style={[styles.emptyState, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <CalendarRange size={48} color={colors.textTertiary} />
            <Text style={[styles.emptyTitle, { color: colors.text }]}>No Monthly Tasks</Text>
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
              No monthly tasks scheduled. Tap + to add one.
            </Text>
          </View>
        ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, { color: colors.text }]}>Add Monthly Task</Text>
            ...
            <Text style={[styles.inputLabel, { color: colors.text }]}>Day of Month</Text>
              ...
              {daysInMonth.map((day) => (
                <Pressable
                  ...
                  style={[
                    styles.dayChip,
                    { borderColor: colors.border },
                    selectedDayOfMonth === day && { backgroundColor: '#6366F1', borderColor: '#6366F1' },
                  ]}
                  onPress={() => setSelectedDayOfMonth(day)}
                >
                  <Text style={[
                    styles.dayChipText,
                    { color: selectedDayOfMonth === day ? '#FFF' : colors.text },
                  ]}>
                    {day}
                  </Text>
                </Pressable>
              ...
              {(Object.keys(AREA_LABELS) as SanitationArea[]).map((area) => (
                <Pressable
                  ...
                  style={[
                    styles.areaChip,
                    { borderColor: colors.border },
                    newTaskArea === area && { backgroundColor: '#6366F1', borderColor: '#6366F1' },
                  ]}
